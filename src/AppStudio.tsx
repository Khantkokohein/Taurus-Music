  ...
  return <div className="min-h-screen bg-[#080812] text-zinc-100">
    ...
    <div className="relative flex min-h-screen">
      <aside className="hidden w-72 border-r border-white/10 bg-white/[0.03] p-6 lg:block">
        ...
        <nav className="mt-10 space-y-2 text-sm">{(['landing','create','history','wallet','plans'] as StudioPage[]).map(x => <button key={x} onClick={() => setActivePage(x)} className={`w-full rounded-2xl px-4 py-3 text-left capitalize ${activePage===x?'bg-[#D4A945] text-black':'bg-white/[0.04]'}`}>{x}</button>)}<button onClick={() => setShowVoiceHub(true)} className="flex w-full items-center gap-2 rounded-2xl bg-violet-500/10 px-4 py-3 text-left text-violet-200"><Mic2 className="h-4 w-4"/>Taurus Voice</button><button onClick={() => setShowDeveloperHub(true)} className="flex w-full items-center gap-2 rounded-2xl bg-white/[0.04] px-4 py-3 text-left"><Code2 className="h-4 w-4"/>Developers</button>{admin && <button onClick={() => setShowAdmin(true)} className="w-full rounded-2xl bg-violet-500/20 px-4 py-3 text-left">Admin</button>}</nav>
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8">
        <header className="mb-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="relative max-w-xl flex-1"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500"/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search songs..." className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-3 pl-11 pr-4 outline-none"/></div><div className="flex gap-3"><div className="rounded-2xl border border-white/10 px-4 py-3 text-sm"><Wallet className="mr-2 inline h-4 w-4"/>{credits}</div>{user ? <button onClick={logout} className="rounded-2xl border border-white/10 px-4 py-3 text-sm"><LogOut className="mr-2 inline h-4 w-4"/>Logout</button> : <button onClick={signInWithGoogle} className="rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-zinc-950"><UserIcon className="mr-2 inline h-4 w-4"/>Gmail</button>}</div></header>
        <div className="mb-6 flex gap-2 overflow-x-auto pb-1 lg:hidden">{(['landing','create','history','wallet','plans'] as StudioPage[]).map(x => <button key={x} onClick={() => setActivePage(x)} className={`shrink-0 rounded-2xl px-4 py-2 text-xs font-bold capitalize ${activePage===x?'bg-[#D4A945] text-black':'border border-white/10 bg-white/[0.04] text-zinc-300'}`}>{x}</button>)}<button onClick={() => setShowVoiceHub(true)} className="shrink-0 rounded-2xl border border-violet-500/20 bg-violet-500/10 px-4 py-2 text-xs font-bold text-violet-200">Voice</button><button onClick={() => setShowDeveloperHub(true)} className="shrink-0 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-2 text-xs font-bold">API</button></div>
