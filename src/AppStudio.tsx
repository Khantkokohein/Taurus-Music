...
const buildPrivateFamilyProfile = (authUser: User, isAdmin: boolean): UserProfile => {
  const today = new Date().toISOString().slice(0, 10);
  const month = today.slice(0, 7);
  const taurusId = buildTaurusAccountCode(authUser.uid);
  return {
    uid: authUser.uid,
    email: '',
    displayName: authUser.displayName || 'Taurus Family',
    taurusId,
    taurusNumber: taurusId,
    taurusCoinBalance: 0,
    songCreditBalance: Number.MAX_SAFE_INTEGER,
...
export default function AppStudio() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async authUser => {
      ...
      const isPrivateFamilyUser = tokenResult.claims.privateFamily === true;
      setTokenAdmin(isTokenAdmin);
      setPrivateFamily(isPrivateFamilyUser);
      if (isPrivateFamilyUser) {
        setProfile(buildPrivateFamilyProfile(authUser, isTokenAdmin));
        setHistory([]);
        return;
      }
  ...
  useEffect(() => {
    if (!user || privateFamily) return undefined;
  ...
  const generate = async () => {
    ...
    try {
      ...
      for (const variant of variants) {
        ...
        const songDetails = {
          id,
          idea: compactTitle(idea, mood, genre, variant.title),
          prompt: compiled,
          mimeType: response.mimeType || blob.type || 'audio/wav',
          lyrics: response.lyrics || lyrics || 'Generated by Taurus Studio.',
          lyriaModel: (response.model as LyriaModelId) || modelForRun,
          instrumentTags: instruments,
          voiceStrength,
          voiceProfileId: selectedVoiceProfile?.id || '',
          voiceProfileName: selectedVoiceProfile?.name || '',
          remixMode,
          remixReferencePath: remixReference?.storagePath || '',
          remixReferenceName: remixReference?.name || '',
        };
        if (privateFamily) {
          const inMemorySong: Song = {
            ...songDetails,
            userId: user.uid,
